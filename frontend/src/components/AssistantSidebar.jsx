...
export default function AssistantSidebar() {
  ...
  const topOffset = anchoredPaths.some((regex) =>
  ...
  const [expanded, setExpanded] = useState(() =>
    getStoredBoolean("assistant:sidebar:expanded", true)
  );
  ...
  const [chatMessages, setChatMessages] = useState(() => [
    {
      id: "welcome",
      author: "assistant",
      text: "¡Hola! Soy tu asistente Edvance. ¿En qué te ayudo hoy?",
    },
  ]);
  ...
  const [notesLessonId, setNotesLessonId] = useState(() =>
    getLessonIdFromStorage()
  );

  const { panelWidth, startResize, wasDraggingRef } = useResizablePanel({
    isExpanded: expanded,
    initialWidth: DEFAULT_ASSISTANT_WIDTH,
    minWidth: DEFAULT_ASSISTANT_WIDTH,
  ...
  useEffect(() => {
    try {
      localStorage.setItem("assistant:sidebar:expanded", expanded ? "1" : "0");
    } catch (_error) {
      // noop
    }
  }, [expanded]);
  ...
  useEffect(() => {
    try {
      ...
      body.style.setProperty("--assistant-rail-width", `${panelWidth}px`);
    } catch (_error) {
      // noop
    }
  }, [expanded, panelWidth]);
    ...
    () => () => {
      try {
        ...
        body.classList.remove(
          ...
          "assistant-rail-expanded",
          "assistant-rail-collapsed"
        );
  ...
  const openQuiz = useCallback(
    (payload) => {
      appendAssistantMessage(
        payload?.message ||
          "Listo, preparo un quiz de 5 preguntas para repasar lo que vimos. Abramos el Salón Virtual para comenzar."
      );
      navigate("/dashboard/salon-virtual");
      try {
        toast.info("Abriendo Quiz en Salón Virtual", 2500);
      } catch (_error) {
        // noop
      }
    },
  ...
  const openExplanation = useCallback(
    (payload) => {
      appendAssistantMessage(
        payload?.message ||
          "Estoy preparando una explicación paso a paso para reforzar los conceptos clave. Veamos el contenido juntos."
      );
      navigate("/dashboard/salon-virtual");
    },
  ...
  const openVirtualRoom = useCallback(() => {
    appendAssistantMessage(
      "Abramos el Salón Virtual para seguir trabajando contigo."
    );
    navigate("/dashboard/salon-virtual");
    try {
      toast.info("Abriendo Salón Virtual", 2500);
    } catch (_error) {
      // noop
    }
  }, [appendAssistantMessage, navigate, toast]);
  ...
  const openLessonNotes = useCallback(
    (payload) => {
      ...
      appendAssistantMessage(
        payload?.message ||
          "Abrí tus notas locales para que captures dudas y aprendizajes cuando quieras."
      );
      ensureExpanded();
    },
  ...
  const openPlan = useCallback(
    (payload) => {
      try {
        const text =
          (payload && (payload.text || payload.objective)) ||
          "Power BI para BI Analyst";
        window.dispatchEvent(
          new CustomEvent("prefill-plan", { detail: { text } })
        );
      } catch (_error) {
        // noop
      }
  ...
  const primaryActions = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Notas",
        emoji: "📝",
        title: "Abrir notas de la lección",
      ...
      },
      {
        ...
        label: "Salón Virtual",
        emoji: "🎥",
        title: "Abrir Salón Virtual",
        onClick: () => openVirtualRoom(),
      },
    ],
    ...
    () => [
      ...
      },
      {
        ...
        onSelect: () =>
          pushSuggestion({
            ...
            bullets: [
              "Video de repaso de 10 minutos",
              "Artículo con ejemplos prácticos",
              "Ejercicio guiado",
            ],
          }),
      },
      {
        ...
        onSelect: () => {
          ...
          try {
            toast.success("Recordatorio configurado en tu panel", 2200);
          } catch (_error) {
            // noop
          }
        },
  ...
  const handleHandleClick = useCallback(() => {
    // Do not toggle if the user was just dragging the handle
    if (wasDraggingRef.current) return;
    // If not expanded, expand it. Otherwise, toggle.
    if (!expanded) {
      ensureExpanded();
      return;
    }
  ...
  useEffect(() => {
    const api = {
      ...
      notify: (s) => {
        try {
          toast.info(s?.title || "Notificación", 2500);
        } catch (_error) {
          // noop
        }
      },
    ...
    try {
      window.Assistant = Object.assign(window.Assistant || {}, api);
    } catch {
      window.Assistant = api;
    }
    return () => {};
  }, [
    ensureExpanded,
  ...
  const moreMenuPlacement = expanded
    ? "right-0 top-full mt-2 origin-top-right"
    : "right-full top-1/2 -translate-y-1/2 -mr-3 origin-right";
  ...
  return (
    ...
    >
      ...
      <header className="h-14 flex items-center px-2 border-b border-slate-200/70 dark:border-slate-700/60">
        <div
          className={`flex items-center w-full ${expanded ? "gap-3 justify-start" : "justify-center"}`}
        >
          <button
            type="button"
          ...
          >
            <span
              className={`inline-flex items-center justify-center ${expanded ? "w-9 h-9" : "w-7 h-7"} rounded-full bg-gradient-to-br from-sky-500 to-sky-600 text-white shadow-md transition-transform duration-300 group-hover:scale-105`}
            >
              <Sparkles size={expanded ? 18 : 16} />
            </span>
            {expanded && (
            ...
            {!showCompactBadge && showExpandedDot && (
              <span
                className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-rose-500 border border-white dark:border-slate-900"
                aria-hidden
              />
            )}
          </button>
      ...
      <div className="h-[calc(100%-3.5rem)] flex flex-col">
        <div
          className={`${expanded ? "px-3 py-3 space-y-2" : "px-1 py-2"} border-b border-slate-200/70 dark:border-slate-700/60`}
          aria-label="Acciones rápidas"
        >
          {expanded && (
            <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-500/80">
              Acciones rápidas
            </p>
          )}
          {expanded ? (
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
            ...
            >
              <ul className="p-3 space-y-3">
                {feed.map((item) => (
                  <li
                    key={item.id}
                    className="rounded-xl border border-slate-200/70 dark.border-slate-700/60 bg-white/80 dark:bg-slate-900/80 backdrop-blur p-3"
                  >
          ...
          {expanded && (
            <div
              className={`${hasFeed ? "mt-auto border-t" : "flex-1 border-t"} border-slate-200/70 dark:border-slate-700/60 bg-white/70 dark:bg-slate-900/70 px-3 py-3 flex flex-col space-y-3`}
            >
              ...
              <div
                ref={chatRef}
                className={`${hasFeed ? "max-h-48" : "flex-1"} overflow-y-auto space-y-2 pr-1 text-sm`}
              >
      ...
      {notesOpen && (
        <div
          className="absolute inset-0 z-50 bg-white/95 dark:bg-slate-900/95 border-l border-slate-200/70 dark:border-slate-700/60 shadow-2xl flex flex-col"
          role="dialog"
          aria-modal="true"
        >
          <header className="flex items-center justify-between px-4 py-3 border-b border-slate-200/70 dark:border-slate-700/60">
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Guardado local por lección · {notesLessonId}
              </p>
            </div>
            <button
              type="button"
