    ...
    >
      ...
      >
        <header className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
          <h2 id="auth-modal-title" className="text-slate-900 font-semibold">
            Acceso
          </h2>
          <button
            onClick={() => {
...
/**
 * Hook para manejar la lógica común de los formularios de autenticación.
 * @param {{
 *   initialState: object,
 *   validationFn: (state: object) => object,
 *   onSubmitFn: (state: object) => Promise<any>,
 *   onSuccess: () => void,
 *   errorFromContext: string | null
 * }} options
 */
function useAuthForm({
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await onSubmitFn(state);
      onSuccess?.();
    } catch (err) {
      toast.error(errorFromContext || err.message || "Ocurrió un error.");
    } finally {
      setSubmitting(false);
    }
  };

  return { state, errors, submitting, handleChange, handleSubmit };
}
  ...
  const { state, errors, submitting, handleChange, handleSubmit } = useAuthForm(
    {
      ...
      validationFn: ({ name, email, password, confirm, terms }) => {
        ...
        if (!isStrongPassword(password))
          e.password = "Mín. 8 caracteres y 1 número";
...
function ResetForm({ firstFieldRef, onSuccess, errorFromContext, onReset }) {
  const { state, errors, submitting, handleChange, handleSubmit } = useAuthForm(
    {
      initialState: { email: "" },
      validationFn: ({ email }) => {
      ...
      onSubmitFn: ({ email }) => onReset(email),
      onSuccess,
      errorFromContext,
    }
  );
...
function GoogleIcon() {
  return (
    ...
    >
      <path
        fill="#FFC107"
        d="M43.611 20.083H42V20H24v8h11.303C33.602 32.243 29.197 36 24 36c-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.153 7.961 3.039l5.657-5.657C33.64 6.053 29.084 4 24 4 12.954 4 4 12.954 4 24s8.954 20 20 20 20-8.954 20-20c0-1.341-.138-2.651-.389-3.917z"
      />
      <path
        fill="#FF3D00"
        d="M6.306 14.691l6.571 4.816C14.292 16.108 18.74 12 24 12c3.059 0 5.842 1.153 7.961 3.039l5.657-5.657C33.64 6.053 29.084 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.132 0 9.63-1.947 13.107-5.129l-6.062-4.995C29.018 35.255 26.641 36 24 36c-5.176 0-9.567-3.736-10.965-8.734l-6.54 5.036C9.792 39.556 16.338 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.611 20.083H42V20H24v8h11.303c-1.357 3.243-4.443 5.799-8.099 6.876l.006-.004 6.062 4.995C31.018 39.255 36 36 36 24c0-1.341-.138-2.651-.389-3.917z"
      />
    </svg>
  );
}
