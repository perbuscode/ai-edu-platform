...
export default function Topbar({ leftOffsetClass = 'left-64', showLogo = false, title }) {
  ...
  useEffect(() => {
    function onKey(e){ if(e.key === 'Escape') setFaqOpen(false); }
    function onClick(e){ if(faqRef.current && !faqRef.current.contains(e.target)) setFaqOpen(false); }
    if (faqOpen) { document.addEventListener('keydown', onKey); document.addEventListener('mousedown', onClick); return () => { document.removeEventListener('keydown', onKey); document.removeEventListener('mousedown', onClick); }; }
  }, [faqOpen]);
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true" className={faqFaqOpen?"rotate-180 transition-transform":"transition-transform"}><path d="M6 9l6 6 6-6" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
                  </button>
                  {faqFaqOpen && (
                    <div id="submenu-faqs" className="px-3 py-1 text-sm text-slate-600 space-y-1">
