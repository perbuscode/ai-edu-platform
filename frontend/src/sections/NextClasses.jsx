...
const defaultItems = [
  { color: "bg-blue-500", title: "Async/Await (Frontend)", when: "Hoy, 3:00 PM" },
  { color: "bg-green-500", title: "Modelado de Datos (Power BI)", when: "Mañana, 10:00 AM" },
  { color: "bg-purple-500", title: "Pruebas unitarias", when: "Viernes, 2:00 PM" },
];

export default function NextClasses({ observe, items }) {
  const ref = useRef(null);
  useEffect(() => (observe ? observe(ref.current) : undefined), [observe]);
  const data = useMemo(() => (items && Array.isArray(items) ? items : defaultItems), [items]);
  ...
  return (
    <section id="proximas" ref={ref} className="scroll-mt-20 bg-white rounded-xl shadow-xl border border-slate-200 p-6 text-slate-900">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold text-slate-900">Próximas clases</h2>
      ...
      <ul className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {data.map((it, i) => (
          <li
            ...
            role="button"
            tabIndex={0}
            onClick={() => navigate('/course', { state: { openTab: 'classroom', classTitle: it.title } })}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); navigate('/course', { state: { openTab: 'classroom', classTitle: it.title } }); } }}
