// src/pages/Dashboard.jsx (legacy)
...
export default function Dashboard() {
  ...
  // Refuerzo: normaliza con cadenas escapadas (evita problemas de encoding del archivo)
  useEffect(() => {
    try {
      const setAttr = (id, attr, val) => { const el = document.getElementById(id); if (el) el.setAttribute(attr, val); };
      const lbl = document.getElementById('sidebar-label'); if (lbl) lbl.textContent = 'Navegaci\u00F3n';
      setAttr('sidebar', 'aria-label', 'Navegaci\u00F3n del curso');
      setAttr('showNavBtn', 'aria-label', 'Mostrar navegaci\u00F3n');
      setAttr('hideSidebar', 'aria-label', 'Ocultar navegaci\u00F3n');
    } catch {}
  }, []);

  // Refuerzo: reemplazo global con escapes Unicode (para textos añadidos dinámicamente)
  useEffect(() => {
    try {
      const pairs = [
      ...
      const applyFix = (root) => {
        ...
        for (const node of nodes) {
          ...
          for (const [badEsc, goodEsc] of pairs) {
            const bad = badEsc.replace(/\\uFFFD/g, '\uFFFD');
            const good = goodEsc.replace(/\\u00([0-9A-Fa-f]{2})/g, (_,h)=> String.fromCharCode(parseInt(h,16)));
            if (t.includes(bad)) { t = t.split(bad).join(good); changed = true; }
          }
          if (changed) node.nodeValue = t;
        }
      };
      const root = document.getElementById('root') || document.body; if (!root) return;
      applyFix(root);
      const mo = new MutationObserver((muts)=>{
        for (const m of muts) {
          m.addedNodes && m.addedNodes.forEach((node)=>{
            if (node.nodeType === Node.TEXT_NODE) applyFix(node.parentNode || root);
            else if (node.nodeType === Node.ELEMENT_NODE) applyFix(node);
          });
        }
      });
  ...
  useEffect(() => {
    ...
    function onKey(e){ if(e.key === 'Escape'){ if (notifOpen) setNotifOpen(false); if (userOpen) setUserOpen(false);} }
  ...
  useEffect(()=>{ if (notifOpen) requestAnimationFrame(()=> notifFirstRef.current?.focus()); }, [notifOpen]);
  useEffect(()=>{ if (userOpen) requestAnimationFrame(()=> userFirstRef.current?.focus()); }, [userOpen]);

  // Corrección de etiquetas en español (forzando tildes correctas)
  useEffect(() => {
    try {
      const lbl = document.getElementById('sidebar-label');
      if (lbl) lbl.textContent = 'Navegación';
      const sb = document.getElementById('sidebar');
      if (sb) sb.setAttribute('aria-label', 'Navegación del curso');
      const showBtn = document.getElementById('showNavBtn');
      if (showBtn) showBtn.setAttribute('aria-label', 'Mostrar navegación');
      const hideBtn = document.getElementById('hideSidebar');
      if (hideBtn) hideBtn.setAttribute('aria-label', 'Ocultar navegación');
    } catch {}
  }, []);

  // Corrección global de textos con tildes mal codificadas en el DOM
  useEffect(() => {
    try {
      const replacements = [
        ['Navegaci\uFFFDn', 'Navegación'],
      ...
      for (const node of nodes) {
        ...
        for (const [bad, good] of replacements) {
          // convertir secuencias \uFFFD a caract. real para comparar
          const badReal = bad.replace('\\uFFFD', '\uFFFD');
          if (t.includes(badReal)) { t = t.split(badReal).join(good); changed = true; }
        }
        if (changed) node.nodeValue = t;
      }
    } catch {}
  ...
  // Normaliza etiquetas en español cuando hubo textos mal codificados
  useEffect(() => {
    try {
      const lbl = document.getElementById('sidebar-label');
      if (lbl) lbl.textContent = 'Navegación';
  ...
  function collapseSidebar() { setSidebarMode('collapsed'); }
  function expandSidebar() { setSidebarMode('expanded'); }
  function hideSidebar() { setSidebarMode('hidden'); }

  function getDisplayName(u){
    if (!u) return 'Invitado';
    if (u.displayName && u.displayName.trim()) return u.displayName.trim();
    if (u.email){ const local = u.email.split('@')[0] || ''; return local.charAt(0).toUpperCase() + local.slice(1); }
    return 'Invitado';
  }
  const displayName = getDisplayName(user);
  const avatarInitial = (displayName || user?.email || 'I').trim().charAt(0).toUpperCase();
  ...
  const coursesMock = useMemo(() => ([
    ...
    { id: 'powerbi', title: 'Master Power BI', progress: 25, hours: '1.2', next: 'Mañana, 10:00 a.m.' },
  ]), []);

  // Fallback de red y estados vacíos (patrón de carga simulada)
  ...
  function retryLoadCourses() {
    ...
    try {
      ...
      if (e?.message === 'offline') toast.offline(); else toast.loadError();
    } finally {
  ...
  return (
    <div className="bg-gray-50 text-gray-900 dark:bg-slate-950 dark:text-slate-100">
          ...
          <nav id="sidebar-content" className="mt-3 p-2" role="tablist" aria-orientation="vertical">
            ...
            <div id="sidebar-course-tabs" className={`${sidebarMode === 'collapsed' ? 'hidden' : 'flex'} flex-col gap-1 px-2 pb-3`}>
              {[
                ...
                <button key={t.key} id={`tab-${t.key}`} onClick={()=>setActiveTab(t.key)} className={`px-4 py-2 rounded-md text-sm text-left ${activeTab === t.key ? 'bg-blue-600 text-white' : 'hover:bg-gray-50 dark:hover:bg-slate-800'}`} role="tab" aria-selected={activeTab===t.key}>
                  {t.label}
                </button>
              ))}
            </div>
      ...
      {/* En mobile, al expandir desplazamos el contenido; no usamos overlay */}

      <div id="app" className={appClass} onClick={mobile && sidebarMode === 'expanded' ? collapseSidebar : undefined}>
        <header className={`bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-gray-200 dark:border-slate-800 px-4 sm:px-6 lg:px-8 py-4 ${view === 'course' ? '' : 'sticky top-0 z-20'}`}>
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div className="pl-2">
                <h1 className="text-lg sm:text-2xl font-bold text-gray-900 dark:text-slate-100 leading-tight">Bienvenido, <span className="text-blue-600">{displayName}</span></h1>
              ...
              <div className="relative">
                ...
                {notifOpen && (
                  <div id="notifPanel" className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-md shadow-lg py-2 z-50">
                    ...
                    <div className="px-3 pt-2"><button ref={notifFirstRef} className="text-xs text-blue-600 dark:text-blue-300 hover:underline">Ver todas</button></div>
                  </div>
                )}
              </div>
              <div className="relative">
                <button id="userMenuBtn" onClick={(e)=>{ e.stopPropagation(); setUserOpen(v=>!v); setNotifOpen(false); }} className="h-10 px-3 rounded-md border border-gray-200 dark:border-slate-700 flex items:center gap-2 hover:bg-gray-50 dark:hover:bg-slate-800" aria-haspopup="menu" aria-expanded={userOpen}>
                  <div className="w-6 h-6 rounded-full bg-gray-200 dark:bg-slate-700 grid place-items-center text-[10px]">{avatarInitial}</div>
                  <div className="hidden sm:block text-left"><p className="text-sm font-medium leading-4">{displayName}</p><p className="text-xs text-gray-500 dark:text-slate-400 leading-4">Estudiante</p></div>
                ...
                {userOpen && (
                  <div id="userMenu" className="absolute right-0 mt-2 w-44 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-md shadow-lg py-1 z-50">
                    <button ref={userFirstRef} onClick={()=>{ setUserOpen(false); navigate('/profile'); }} className="w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-slate-800">Perfil</button>
        ...
        <main id="main" className="p-4 sm:p-6 lg:p-8">
          ...
          <section id="dashboard-view" role="tabpanel" className={view === 'dashboard' ? '' : 'hidden'}>
            ...
            {coursesError && (
              <div className="rounded-xl border border-dashed border-gray-300 dark:border-slate-700 p-6 text-center mb-6">
                ...
                <p className="text-sm text-gray-700 dark:text-slate-300">No pudimos cargar tus cursos.</p>
                <p className="text-xs text-gray-500 dark:text-slate-400 mb-3">Verifica tu conexión y vuelve a intentarlo.</p>
                <button onClick={retryLoadCourses} className="px-4 py-2 rounded-lg text-sm bg-slate-900 text-white hover:bg-slate-800">Reintentar</button>
              </div>
            )}
            {!coursesError && !coursesLoading && coursesData.length === 0 && (
              <div className="rounded-xl border border-dashed border-gray-300 dark:border-slate-700 p-6 text-center mb-6" role="status" aria-live="polite">
                <div className="text-2xl mb-1" aria-hidden>📚</div>
                <p className="text-sm text-gray-700 dark:text-slate-300">Aún no tienes cursos activos.</p>
                <p className="text-xs text-gray-500 dark:text-slate-400">Cuando te inscribas, aparecerán aquí.</p>
              </div>
            )}
              ...
              {(!coursesError ? coursesData : []).map((c) => (
                <article key={c.id} className="course-card bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6 card-hover" data-hours={c.hours} data-progress={c.progress} data-id={c.id}>
                  <div className="flex items-center gap-3 mb-3">
                    <div className="w-12 h-12 gradient-bg rounded-lg grid place-items-center" aria-hidden="true">
                      <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3"/></svg>
                    </div>
                    <div>
                      <p className="text-base font-semibold">{c.title}</p>
                      <p className="text-xs text-gray-500 dark:text-slate-400">{c.id === 'frontend' ? 'Módulo 3: JS Avanzado' : 'Nivel: Básico â€“ Avanzado'}</p>
                    </div>
                  </div>
                  <div className="mb-3">
                    <div className="flex items-center justify-between mb-1"><span className="text-sm text-gray-600 dark:text-slate-300">Progreso</span><span className="text-sm font-medium text-green-600">{c.progress}%</span></div>
                    <div className="w-full bg-gray-200 dark:bg-slate-800 rounded-full h-2" aria-hidden="true"><div className="bar-green h-2 rounded-full" style={{ width: `${c.progress}%` }} /></div>
                  </div>
                  <ul className="text-sm text-gray-600 dark:text-slate-300 mb-4">
                    <li>{'\u23F1\uFE0F'} {c.id === 'frontend' ? '2h 30m' : '1h 12m'} restantes</li>
                    <li>{'\uD83D\uDD52'} Próxima: {c.next}</li>
                  </ul>
                  <div className="flex items-center gap-2">
          ...
          <section id="course-view" aria-live="polite" className={view === 'course' ? '' : 'hidden'}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <article className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6 lg:col-span-2">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    ...
                    <li className="border chip dark:border-slate-700 rounded-lg px-3 py-2">{'\u23F1\uFE0F'} Hora estimada de llegada: <span id="ov-eta" className="font-medium">—</span></li>
                  </ul>
                </article>
              </div>
            </section>
            ...
            <section id="tab-studyplan" className={activeTab === 'studyplan' ? '' : 'hidden'}>
              <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6">
                <h3 className="text-lg font-semibold mb-4">Plan de estudio</h3>
                <ol className="space-y-3">
                  <li className="flex items-center justify-between border chip dark:border-slate-700 rounded-lg px-3 py-2"><span>1. IntroducciÃ³n</span><span className="text-xs px-2 py-1 rounded bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300">Completado</span></li>
                  <li className="flex items-center justify-between border chip dark:border-slate-700 rounded-lg px-3 py-2"><span>2. Fundamentos</span><span className="text-xs px-2 py-1 rounded bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">En curso</span></li>
                  <li className="flex items-center justify-between border chip dark:border-slate-700 rounded-lg px-3 py-2"><span>3. Proyecto prÃ¡ctico</span><span className="text-xs px-2 py-1 rounded bg-gray-100 text-gray-700 dark:bg-slate-700 dark:text-slate-200">Pendiente</span></li>
                </ol>
              </div>
            </section>
            <section id="tab-materials" className={activeTab === 'materials' ? '' : 'hidden'}>
              <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6">
                <h3 className="text-lg font-semibold mb-4">Materiales del curso</h3>
                <ul className="space-y-3">
                  <li className="flex items-center gap-3"><div className="w-8 h-8 bg-red-100 dark:bg-red-900/30 rounded-lg grid place-items-center"><span className="text-xs font-medium text-red-600 dark:text-red-300">PDF</span></div><div className="flex-1"><p className="text-sm font-medium">GuÃ­a de conceptos</p><p className="text-xs text-gray-500 dark:text-slate-400">2.3 MB</p></div><button className="text-blue-600 dark:text-blue-300 text-sm font-medium">Descargar</button></li>
                  <li className="flex items-center gap-3"><div className="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-lg grid place-items-center"><span className="text-xs font-medium text-blue-600 dark:text-blue-300">ZIP</span></div><div className="flex-1"><p className="text-sm font-medium">Dataset + soluciones</p><p className="text-xs text-gray-500 dark:text-slate-400">5.7 MB</p></div><button className="text-blue-600 dark:text-blue-300 text-sm font-medium">Descargar</button></li>
                  <li className="flex items-center gap-3"><div className="w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-lg grid place-items-center"><span className="text-xs font-medium text-green-600 dark:text-green-300">MP4</span></div><div className="flex-1"><p className="text-sm font-medium">Clip de apoyo</p><p className="text-xs text-gray-500 dark:text-slate-400">12 min</p></div><button className="text-blue-600 dark:text-blue-300 text-sm font-medium">Ver</button></li>
                </ul>
              </div>
            </section>
          </section>
