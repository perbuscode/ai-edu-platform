...
export default function Dashboard() {
  ...
  // Derivar nombre visible y avatar desde AuthContext
  ...
  const courses = useMemo(() => ([
    ...
    { id: 'powerbi', title: 'Master Power BI', progress: 25, hours: '1.2', next: 'MaÃ±ana, 10:00 AM' },
  ]), []);
  ...
  return (
    <div className="bg-gray-50 text-gray-900 dark:bg-slate-950 dark:text-slate-100">
      {/* Sidebar */}
      {sidebarMode !== 'hidden' && (
        <aside id="sidebar" className={`fixed inset-y-0 left-0 bg-white dark:bg-slate-900 border-r border-gray-200 dark:border-slate-800 z-40 ${sidebarMode === 'collapsed' ? 'collapsed' : 'expanded'}`} aria-label="NavegaciÃ³n del curso" aria-hidden={sidebarMode === 'hidden'}>
          ...
          <nav id="sidebar-content" className="mt-3 p-2" role="tablist" aria-orientation="vertical">
            ...
            <div id="sidebar-course-tabs" className={`${sidebarMode === 'collapsed' ? 'hidden' : 'flex'} flex-col gap-1 px-2 pb-3`}>
              {[
                ...
                { key: 'classroom', label: 'SalÃ³n de Clases' },
            ...
            <div id="sidebar-back-wrap" className={`${sidebarMode === 'collapsed' ? 'hidden' : ''} px-2`}>
              <button id="btn-back-dashboard" onClick={showDashboard} className="mt-2 w-full text-left px-4 py-2 rounded-md bg-slate-100 hover:bg-slate-200 text-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-100">â† Volver al Dashboard</button>
            </div>
          </nav>
        </aside>
      )}
        ...
        <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur border-b border-gray-200 dark:border-slate-800 px-4 sm:px-6 lg:px-8 py-4 sticky top-0 z-20">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <button id="themeBtn" onClick={toggleTheme} className="h-10 px-3 rounded-md border border-gray-200 dark:border-slate-700 flex items-center justify-center hover:bg-gray-50 dark:hover:bg-slate-800" aria-label="Cambiar tema"><span id="themeIcon">{theme === 'dark' ? 'â˜€ï¸' : 'ðŸŒ™'}</span></button>
              <div className="relative">
                ...
                {notifOpen && (
                  <div id="notifPanel" className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-md shadow-lg py-2 z-50">
                    ...
                    <div className="px-3 py-2 hover:bg-gray-50 dark:hover:bg-slate-800"><p className="text-sm">Nueva lecciÃ³n disponible: <span className="font-medium">Async/Await</span></p><p className="text-xs text-gray-500 dark:text-slate-400">Hoy, 2:45 PM</p></div>
                    <div className="px-3 py-2 hover:bg-gray-50 dark:hover:bg-slate-800"><p className="text-sm">Recordatorio: Quiz de Power BI</p><p className="text-xs text-gray-500 dark:text-slate-400">MaÃ±ana, 9:00 AM</p></div>
              ...
              <div className="relative">
                ...
                {userOpen && (
                  <div id="userMenu" className="absolute right-0 mt-2 w-44 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-md shadow-lg py-1 z-50">
                    ...
                    <button onClick={handleLogout} className="w-full text-left px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">Cerrar sesiÃ³n</button>
                  </div>
                )}
              </div>
            </div>
        ...
        <main id="main" className="p-4 sm:p-6 lg:p-8">
          ...
          <section id="dashboard-view" role="tabpanel" className={view === 'dashboard' ? '' : 'hidden'}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6 card-hover"><p className="text-sm text-gray-500 dark:text-slate-400">PrÃ³xima clase</p><p className="text-2xl font-semibold mt-1" id="kpi-next">Hoy, 3:00 PM</p></div>
            </div>
            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6 mb-8 card-hover">
              <div className="flex items-center justify-between mb-4"><h2 className="text-lg font-semibold">PrÃ³ximas clases</h2><div className="text-sm text-gray-500 dark:text-slate-400">Zona horaria local</div></div>
              <ul className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <li className="border border-gray-200 dark:border-slate-800 rounded-lg p-4 flex items-center gap-3"><span className="w-2 h-2 bg-green-500 rounded-full" /><div className="flex-1"><p className="text-sm font-medium">Modelado de Datos (Power BI)</p><p className="text-xs text-gray-500 dark:text-slate-400">MaÃ±ana, 10:00 AM</p></div></li>
            ...
            <div id="courses-grid" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {courses.map((c) => (
                <article key={c.id} className="course-card bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6 card-hover" data-hours={c.hours} data-progress={c.progress} data-id={c.id}>
                  ...
                  <ul className="text-sm text-gray-600 dark:text-slate-300 mb-4"><li>â±ï¸ {c.id === 'frontend' ? '2h 30m' : '1h 12m'} restantes</li><li>ðŸ“… PrÃ³xima: {c.next}</li></ul>
                  <div className="flex items-center gap-2">
          ...
          <section id="course-view" aria-live="polite" className={view === 'course' ? '' : 'hidden'}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <article className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6 lg:col-span-2">
                  ...
                  <ul className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    <li className="border chip dark:border-slate-700 rounded-lg px-3 py-2">â±ï¸ Horas restantes: <span id="ov-hours" className="font-medium">0h</span></li>
                    <li className="border chip dark:border-slate-700 rounded-lg px-3 py-2">ðŸ“š LecciÃ³n actual: <span id="ov-lesson" className="font-medium">â€”</span></li>
                    <li className="border chip dark:border-slate-700 rounded-lg px-3 py-2">ðŸ“… PrÃ³xima clase: <span id="ov-next" className="font-medium">â€”</span></li>
                    <li className="border chip dark:border-slate-700 rounded-lg px-3 py-2">ðŸ ETA final: <span id="ov-eta" className="font-medium">â€”</span></li>
                  </ul>
                </article>
              </div>
            </section>
            <section id="tab-classroom" className={activeTab === 'classroom' ? '' : 'hidden'}>
              <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6">
                ...
                {step === 2 && (
                  <div id="class-step-2"><h3 className="text-lg font-semibold mb-3">Contenido de la clase</h3><div className="prose max-w-none text-gray-800 dark:text-slate-200"><p>En esta lecciÃ³n profundizamos en <strong>conceptos clave</strong> y buenas prÃ¡cticas.</p></div><div className="mt-4 flex items-center gap-2"><button id="btn-step-2-prev" onClick={()=>setStep(1)} className="border chip dark:border-slate-700 px-4 py-2 rounded-lg text-sm">Volver al video</button><button id="btn-step-2-next" onClick={()=>setStep(3)} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium">Siguiente: Ejercicio</button></div></div>
                )}
                {step === 3 && (
                  <div id="class-step-3"><h3 className="text-lg font-semibold mb-3">Ejercicio guiado</h3><div className="border border-dashed border-gray-300 dark:border-slate-700 rounded-lg p-4"><p className="text-sm mb-2">Resuelve el reto y envÃ­a tu respuesta:</p><textarea className="w-full border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 rounded-lg px-3 py-2 text-sm" rows={5} placeholder="Escribe aquÃ­ tu soluciÃ³n, pasos o reflexiÃ³n..." /></div><div className="mt-4 flex items-center gap-2"><button id="btn-step-3-prev" onClick={()=>setStep(2)} className="border chip dark:border-slate-700 px-4 py-2 rounded-lg text-sm">Volver a contenido</button><button id="btn-finish-step" onClick={()=>{ alert('Â¡Clase marcada como completada! AvanzarÃ¡s a la siguiente cuando estÃ© disponible.'); setStep(1); }} className="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-lg text-sm font-medium">Marcar clase como completada</button></div></div>
                )}
              </div>
            </section>
            <section id="tab-studyplan" className={activeTab === 'studyplan' ? '' : 'hidden'}><div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6"><h3 className="text-lg font-semibold mb-4">Plan de estudio</h3><ol className="space-y-3"><li className="flex items-center justify-between border chip dark:border-slate-700 rounded-lg px-3 py-2"><span>1. IntroducciÃ³n</span><span className="text-xs px-2 py-1 rounded bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300">Completado</span></li><li className="flex items-center justify-between border chip dark:border-slate-700 rounded-lg px-3 py-2"><span>2. Fundamentos</span><span className="text-xs px-2 py-1 rounded bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">En curso</span></li><li className="flex items-center justify-between border chip dark:border-slate-700 rounded-lg px-3 py-2"><span>3. Proyecto prÃ¡ctico</span><span className="text-xs px-2 py-1 rounded bg-gray-100 text-gray-700 dark:bg-slate-700 dark:text-slate-200">Pendiente</span></li></ol></div></section>
            <section id="tab-materials" className={activeTab === 'materials' ? '' : 'hidden'}><div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl p-6"><h3 className="text-lg font-semibold mb-4">Materiales del curso</h3><ul className="space-y-3"><li className="flex items-center gap-3"><div className="w-8 h-8 bg-red-100 dark:bg-red-900/30 rounded-lg grid place-items-center"><span className="text-xs font-medium text-red-600 dark:text-red-300">PDF</span></div><div className="flex-1"><p className="text-sm font-medium">GuÃ­a de conceptos</p><p className="text-xs text-gray-500 dark:text-slate-400">2.3 MB</p></div><button className="text-blue-600 dark:text-blue-300 text-sm font-medium">Descargar</button></li><li className="flex items-center gap-3"><div className="w-8 h-8 bg-blue-100 dark:bg-blue-900/30 rounded-lg grid place-items-center"><span className="text-xs font-medium text-blue-600 dark:text-blue-300">ZIP</span></div><div className="flex-1"><p className="text-sm font-medium">Dataset + soluciones</p><p className="text-xs text-gray-500 dark:text-slate-400">5.7 MB</p></div><button className="text-blue-600 dark:text-blue-300 text-sm font-medium">Descargar</button></li><li className="flex items-center gap-3"><div className="w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-lg grid place-items-center"><span className="text-xs font-medium text-green-600 dark:text-green-300">MP4</span></div><div className="flex-1"><p className="text-sm font-medium">Clip de apoyo</p><p className="text-xs text-gray-500 dark:text-slate-400">12 min</p></div><button className="text-blue-600 dark:text-blue-300 text-sm font-medium">Ver</button></li></ul></div></section>
          </section>
        </main>
      </div>
    </div>
