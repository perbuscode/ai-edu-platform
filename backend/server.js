...
function generatePlanLocal({ course, experience, hoursPerDay, weeks, goal }) {
  ...
  const level = /no|cero|0/i.test(experience || '') ? 'Principiante' : (/junior|algo|poco/i.test(experience || '') ? 'Junior' : 'Intermedio');
  const blocks = [
    { title: 'Fundamentos', bullets: ['Conceptos clave', 'Herramientas', 'Buenas prácticas'], project: 'Mini proyecto base', role: 'Jr.' },
    { title: 'Práctica guiada', bullets: ['Técnicas esenciales', 'Proyecto incremental', 'Feedback'], project: 'Proyecto intermedio', role: 'Jr./Mid' },
    { title: 'Aplicación real', bullets: ['Integración', 'Optimización', 'Entrega'], project: 'Proyecto final', role: 'Mid' }
  ];
  const rubric = [
